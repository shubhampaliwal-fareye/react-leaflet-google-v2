...
var google; // class GridLayer.GoogleMutant
// extends GridLayer
...
L.GridLayer.GoogleMutant = L.GridLayer.extend({
  ...
  initialize: function initialize(options) {
    ...
    var self = this;
    _googleMaps.default.VERSION = options.version || "3.43";

    if (options.asclientid) {
      _googleMaps.default.CLIENT = options.googlekey;
    } else {
      _googleMaps.default.KEY = options.googlekey;
    }

    _googleMaps.default.LIBRARIES = options.libraries || [];

    if (options.channel) {
      _googleMaps.default.CHANNEL = options.channel;
    }

    self._type = options.maptype || "SATELLITE";

    _googleMaps.default.load(function (_google) {
      google = _google;
      self._ready = true; // self._initMapObject();
      // self.
  ...
  onAdd: function onAdd(map) {
    ...
    this._initMutantContainer(); // Will be called after callback passed in initialize function
    ...
    _googleMaps.default.load(function (google) {
      ...
      map.on("resize", this._resize, this); // 20px instead of 1em to avoid a slight overlap with google's attribution
  ...
  _initMutantContainer: function _initMutantContainer() {
    if (!this._mutantContainer) {
      ...
      this._mutantContainer.id = "_MutantContainer_" + L.Util.stamp(this._mutantContainer); // 			this._mutantContainer.style.zIndex = 'auto';
  ...
  _initMutant: function _initMutant() {
    ...
    this._mutant = map; // 🍂event spawned
    ...
    if (coords) {
      ...
      if (key in this._tileCallbacks && this._tileCallbacks[key]) {
        ...
        if (parent) {
          ...
          parent.removeChild = L.Util.falseFn; // 					imgNode.parentNode.replaceChild(L.DomUtil.create('img'), imgNode);
        }
  ...
  _createSingleTile: function createTile(coords, done) {
    var key = this._tileCoordsToKey(coords); // console.log('Need:', key);
    ...
    if (key in this._freshTiles) {
      ...
      L.Util.requestAnimFrame(done); // 			console.log('Got ', key, ' from _freshTiles');
    ...
    /* , k*/
      ...
      return function (imgNode) {
        ...
        done(); // 					console.log('Sent ', k, ' to _tileCallbacks');
      };
    }(tileContainer
    /* , key*/
    ));
  ...
  _removeTile: function _removeTile(key) {
    if (this._imagesPerTile > 1) {
      ...
      delete this._freshTiles[key]; // 				console.log('Pruned spurious _freshTiles', key);
    }
